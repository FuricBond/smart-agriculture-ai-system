...
import { Activity, Droplets, Map, ArrowRight } from 'lucide-react';

const FeatureCard = ({ icon: Icon, title, desc, to, color, delay }) => (
    <motion.div
    ...
    >
        {/* Glow behind icon */}
        <div className={`absolute -top-10 -left-10 w-40 h-40 bg-${color}-500 blur-[80px] opacity-20 group-hover:opacity-40 transition-opacity`} />
        ...
        <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-8 bg-white/10 shadow-[inset_0_0_20px_rgba(255,255,255,0.2)] border border-white/20 relative z-10 group-hover:scale-110 transition-transform`}>
            <Icon className={`w-8 h-8 text-${color}-400 drop-shadow-[0_0_10px_rgba(255,255,255,0.8)]`} />
        </div>
        ...
        <Link
            to={to}
            className={`mt-auto flex items-center text-sm font-semibold text-${color}-400 hover:text-${color}-300 relative z-10 uppercase tracking-widest`}
        >
...
const Home = () => {
    return (
        ...
        >
            ...
            >
                <motion.div
                    animate={{ boxShadow: ["0 0 10px rgba(16,185,129,0.3)", "0 0 30px rgba(16,185,129,0.7)", "0 0 10px rgba(16,185,129,0.3)"] }}
                ...
                >
                    System Online // VisionOS Enabled
                </motion.div>

                <h1 className="text-5xl md:text-7xl font-extrabold text-white leading-tight mb-8 tracking-tighter">
                    Cultivate in <br className="hidden md:block" />
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-300 via-emerald-200 to-emerald-600 drop-shadow-[0_0_30px_rgba(16,185,129,0.4)]">
                        Zero Gravity.
                    </span>
                </h1>

                <p className="text-lg md:text-xl text-slate-400 mb-12 max-w-2xl mx-auto leading-relaxed font-light">
                    Harness the power of neural networks to predict diseases, optimize yields, and deploy highly tailored telemetry-driven crop recommendations.
                </p>
                ...
                >
                    <Link to="/report" className="btn-primary flex items-center gap-3">
                        ...
                        <span>Launch Smart System Report</span>
                    </Link>
                </motion.div>
            </motion.div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
                <FeatureCard
                    icon={Activity}
                    title="Diagnostics"
                    desc="Upload telemetry images and let our ResNet-50 core instantly map plant health anomalies."
                    to="/disease"
                    color="rose"
                    delay={0.2}
                />
                <FeatureCard
                    icon={Droplets}
                    title="Optimization"
                    desc="Calibrate soil chemistry and atmospheric conditions to identify optimal biosphere crops."
                    to="/crop"
                    color="indigo"
                    delay={0.4}
                />
                <FeatureCard
                    icon={Map}
                    title="Forecasting"
                    desc="Project season yields using historical data models fused with spatial region analysis."
                    to="/yield"
                    color="amber"
                    delay={0.6}
                />
